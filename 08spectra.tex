...
We will now focus on the spectra of operators and on the decomposition
of the spectra of self-adjoint operators. The significance of spectra
is that the axioms of quantum mechanics prescribe that the possible
measurement values of an observable (which is, in particular, a
self-adjoint operator) are those in the so-called spectrum of the
operator.

A common task in almost any quantum mechanical problem that you might
wish to solve is to determine the spectrum of some observable. This is
usually the Hamiltonian, or energy operator, since the time evolution
of a quantum system is governed by the exponential of the Hamiltonian,
which is more practically determined by first determining its
spectrum.

More often than not, it is not possible to determine the spectrum of
an operator exactly (i.e.\ analytically). One then resorts to
perturbation theory which consists in expressing the operator whose
spectrum we want to determine as the sum of an operator whose spectrum
can be determined analytically and another whose contribution is
``small'' in some sense to be made precise.

\section{Resolvent map and spectrum}

If \(B\colon\Cal D_B\to\Cal H\) is a densely defined operator,
we say \(B\) has a bounded inverse if \(B\) is a linear
isomorphism and the linear inverse \(B^{-1}\colon\Cal H\to\Cal D_B\)
is bounded.
(Note that this does \emph{not} imply that \(B\) is bounded, unless
\(\Cal D_B=\Cal H\), see \cref{rmk:resolventbounded}).
In this case, the \emph{bounded inverse} of \(B\) is the operator
\(B'\colon\Cal H\to\Cal H\) obtained by composing \(B^{-1}\colon\Cal
H\to\Cal D_B\) with the inclusion \(\Cal D_B\to\Cal H\).

Equivalently, a bounded inverse of \(B\) is, if it exists, a bounded
operator \(B'\colon\Cal H\to\Cal H\) such that \(B'(\Cal H)\subseteq
\Cal D_B\), \(BB'=\id_{\Cal H}\) and \(B'B=i_{\Cal D_B}\colon\Cal
D_B\to\Cal H\) is the natural inclusion.

We will use the notation $\mathcal{L}(\mathcal{H})\equiv
\mathcal{L}(\mathcal{H},\mathcal{H})$.
\begin{definition}
  The \emph{resolvent set} of an operator $A\colon\Cal D_A\to\Cal H$
  is the set
  \begin{equation*}
    \rho(A):=\{z\in\C \mid (A-z)\colon\Cal D_A\to\Cal H
      \text{ has a bounded inverse }
    (A-z)'\in \mathcal{L}(\mathcal{H})\}
  \end{equation*}
  The \emph{resolvent map}\index{resolvent map} of $A$ is the map
  \begin{IEEEeqnarray*}{rrCl}
  R_A\cl & \rho(A)& \to & \mathcal{L}(\mathcal{H})\\
  & z & \mapsto & (A-z)'.
  \end{IEEEeqnarray*}
  The \emph{spectrum} of \(A\) is the complement
  \(\sigma(A)=\C-\rho(A)\).
\end{definition}

\begin{remark}
  \label[remark]{rmk:resolventbounded}
  Note that an operator \(B\colon\Cal H\to\Cal H\) has a bounded
  inverse iff it is a linear isomorphism with bounded inverse.
  In this case \(B'=B^{-1}\colon\Cal H\to\Cal H\) and,
  by the Banach isomorphism theorem, \(B\) itself is bounded.
  Hence, the resolvent set of an operator \(A\colon\Cal H\to\Cal H\)
  is
  \begin{equation}
    \rho(A)
    =
    \{
      z\in\Cal C
      \mid
      (A-z)\colon\Cal D_A\to\Cal H
      \emph{ is a bounded linear isomorphism}
    \}
  .\end{equation}
  %The same conclusion holds for a closed operator
  %\(A\colon\Cal D_A\to\Cal H\). Indeed, in this case all the operators
  %\((A-z)\colon\Cal D_A\to\Cal H\) are closed, so
  %whenever they have a
  %by
  %the closed graph theorem, whenever
  %states that a map \(B\colon\Cal H\to\Cal H\) is bounded iff it is
  %closed, so whenever \((A-z)\colon\Cal A\)
\end{remark}
...
\begin{proof}
If $\lambda$ is an eigenvalue of $A$, then there exists $\psi\in\mathcal{D}_A\setminus\{0\}$ such that $A\psi = \lambda \psi$, i.e.\ \begin{equation*}
(A-\lambda)\psi = 0.
\end{equation*}
...
\section{The spectrum of a self-adjoint operator}
...
\begin{proof}
For the implication (\(\impliedby\)), suppose that $\lambda$ is an
eigenvalue of $A$. Then, by self-adjointness of $A$,
\begin{equation*}
...
For the converse implication (\(\implies\)), we now need to show that
if $\lambda\in\sigma_{\mathrm{p}}(A)$, then $\lambda$ is an eigenvalue
of $A$. By contraposition, suppose that $\lambda\in\C$ is not an
eigenvalue of $A$. Note that if $\lambda$ is real, then $\lambda =
\overline{\lambda}$ while if $\lambda$ is not real, then
$\overline{\lambda}$ is not real. Hence, if $\lambda$ is not an
eigenvalue of $A$, then neither is $\overline{\lambda}$. Therefore,
there exists no non-zero $\psi$ in $\mathcal{D}_A$ such that
$A\psi=\overline{\lambda}\psi$. Thus, we have
\begin{equation*}
...
\section{Perturbation theory for point spectra of self-adjoint operators}
...
\subsection{Unperturbed spectrum}
...
\subsection{Formal power series ansatz}
%\footnote{German for ``educated guess''.}}
...
\subsection{Fixing phase and normalisation of perturbed eigenvectors}
...
\subsection{Order-by-order decomposition of the perturbed eigenvalue problem}
%\addcontentsline{toc}{section}{D. \ Order-by-order decomposition of the perturbed eigenvalue problem}

Let us insert our formal power series ansatz into the perturbed eigenvalue equation. On the left-hand side, we find
\begin{align*}
H_{\lambda}e_{n\delta}(\lambda)
& = (H_0+\lambda W)(e_{n\delta} + \lambda\epsilon_{n\delta}^{(1)} + \lambda^2\epsilon_{n\delta}^{(2)}+\mathcal{O}(\lambda^3))\\
...
\begin{align*}
&\hspace{-5mm}
h_{n\delta}(\lambda)e_{n\delta}(\lambda) \\
& = (h_n + \lambda\theta_{n\delta}^{(1)} + \lambda^2\theta_{n\delta}^{(2)}+\mathcal{O}(\lambda^3)) (e_{n\delta} + \lambda\epsilon_{n\delta}^{(1)} + \lambda^2\epsilon_{n\delta}^{(2)}+\mathcal{O}(\lambda^3))\\
...
\subsection{Second-order eigenvalue correction}
...
\begin{remark}
Note that, while the first-order correction to the perturbed $n\delta$
eigenvalue only depends on the unperturbed $n\delta$ eigenvalue and
eigenvector, the second-order correction draws information from
\emph{all} the unperturbed eigenvalues and eigenvectors. Hence, if we
try to approximate a relativistic system as a perturbation of a
non-relativistic system, then the second-order corrections may be
unreliable.
\end{remark}
