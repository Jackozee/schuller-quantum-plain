...
\begin{equation*}
\psi_{\parallel} := \langle e| \psi \rangle e
\end{equation*}
while the \emph{orthogonal complement} of $\psi$ is
\begin{equation*}
\psi_{\perp}:=\psi-\psi_{\parallel}.
\end{equation*}
\end{definition}
...
\item we can write $\psi = \psi_{\parallel}+\psi_{\perp}$, where
\begin{equation*}
\psi_{\parallel}:= \sum_{i=0}^{\infty}\langle e_i | \psi \rangle e_i, \qquad \psi_{\perp} :=\psi-\psi_{\parallel}
\end{equation*}
and we have
\begin{equation*}
...
\begin{equation*}
\|\psi\|^2 = \|\psi_{\parallel}\|^2+\|\psi_{\perp}\|^2.
...
\end{equation*}
with equality if, and only if, $\gamma =\psi_{\parallel}$.
\end{enumerate}
\end{proposition}
...
\begin{enumerate}[label=(\alph*)]
\item Let $\psi_{\parallel}$ and $\psi_{\perp}$ be defined as in the proposition. Then $\psi_{\parallel}+\psi_{\perp}=\psi$ and 
...
\begin{equation*}
\langle \psi_{\perp} | \psi_{\parallel}\rangle = \biggl\langle\psi_{\perp} \,\bigg|\,  \sum_{i=0}^{n}\langle e_i | \psi \rangle e_i \biggr\rangle = \sum_{i=0}^{n}\langle e_i | \psi \rangle \langle \psi_{\perp} | e_i\rangle = 0.
\end{equation*}
Hence, by (the finite-dimensional) Pythagoras' theorem
\begin{equation*}
\|\psi\|^2 = \|\psi_{\parallel}+\psi_{\perp}\|^2= \|\psi_{\parallel}\|^2+\|\psi_{\perp}\|^2.
\end{equation*}
\item Let $\gamma\in\lspan\{e_i\mid 0\leq i\leq n \}$. Then $\gamma = \sum_{i=0}^n\gamma_ie_i$ for some $\gamma_0,\ldots,\gamma_n\in\C$. Hence
\begin{IEEEeqnarray*}{rCl}
\|\psi-\gamma \|^2 & = & \|\psi_{\perp}+\psi_{\parallel}-\gamma \|^2\\
...
\end{IEEEeqnarray*}
and thus $\|\psi-\gamma\| \geq \|\psi_{\perp}\|$ since $|\langle e_i | \psi \rangle -\gamma_i|^2>0$ for all $0\leq i\leq n$. Moreover, we have equality if, and only if, $|\langle e_i | \psi \rangle -\gamma_i|=0$ for all $0\leq i\leq n$, that is $\gamma = \psi_{\parallel}$.
\end{enumerate}
...
\begin{equation*}
\psi_{\parallel}:=\sum_{i=0}^{\infty}\langle e_i | \psi \rangle e_i
\end{equation*}
...
\section{Closed linear subspaces}
...
Hence
\begin{equation*}
\mathrm{P}_{\!\mathcal{M}^{\perp}}\psi =\psi-\mathrm{P}_{\!\mathcal{M}}\psi = \psi-\psi_{\parallel} =: \psi_{\perp}.
\end{equation*}
\item Let $\psi\in \mathcal{H}$. Then, by Pythagoras' theorem,
\begin{equation*}
\sup_{\psi\in\mathcal{H}}\frac{\|\mathrm{P}_{\!\mathcal{M}}\psi\|}{\|\psi\|} = \sup_{\psi\in\mathcal{H}}\frac{\|\psi_{\parallel}\|}{\|\psi\|} =  \sup_{\psi\in\mathcal{H}}\frac{\|\psi\|-\|\psi_{\perp}\|}{\|\psi\|} \leq 1 <\infty \qedhere
\end{equation*}
\end{enumerate}
\end{proof}
...
\begin{equation*}
 \mathrm{P} =  i\circ \mathrm{P}_{ \mathrm{P}(\mathcal{H})}.
\end{equation*}
where \(i\colon \mathrm{P}(\Cal H)\to \Cal H\) is the natural inclusion.
In other words, every projector is the orthogonal projector to some
closed linear subspace.
\end{theorem}

\section{Riesz representation theorem, bras and kets}

Let $\mathcal{H}$ be a Hilbert space.
For every \(\phi\in\Cal H\), consider the map
\begin{IEEEeqnarray*}{rrCl}
f_{\varphi}\cl & \mathcal{H} & \to & \C\\
&\psi & \mapsto & \langle \varphi | \psi \rangle .
\end{IEEEeqnarray*}
The linearity in the second argument of the inner product implies that
this map is linear. Moreover, by the Cauchy-Schwarz inequality, we
have
\begin{equation*}
|f_{\varphi}(\psi)|
...
\leq \|\varphi \|\| \psi \|
\end{equation*}
Hence, $f_{\varphi}\in\mathcal{L}(\mathcal{H},\C)=:\mathcal{H}^*$ with
norm \(\|f_\phi\|\leq\|\phi\|\).
In fact, taking \(\psi=\phi\) we get
\(\|\phi\|^{2}=\|f_\phi(\phi)\|\leq\|f_\phi\|\|\phi\|\), so
\(\|\phi\|\leq\|f_\phi\|\) as well.
Therefore, to every element of $\varphi$ of $\mathcal{H}$, there is
associated an element $f_{\varphi}$ in the dual space $\mathcal{H}^*$
with the same norm \(\|f_\phi\|=\|\phi\|\).
In fact, we have the following theorem.

\begin{theorem}[Riesz representation]
    \label{thm:riesz}
    Every $f\in\mathcal{H}^*$ is of the form $f_{\varphi}$ for a
    unique $\varphi\in\mathcal{H}$.
\end{theorem}
...
\begin{equation*}
|\psi \rangle
= \sum_{i=0}^{\infty}\langle e_i | \psi \rangle |e_i\rangle.
\end{equation*}
By allowing the scalar multiplication of kets also from the
right, defined to yield the same result as that on the left, we have
\begin{equation*}
|\psi \rangle
...
\end{equation*}
where by ``quite obviously'', we mean that we have a suppressed tensor
product
\begin{equation*}
|\psi \rangle
 = \biggl(\sum_{\,i=0}^{\infty}|e_i\rangle\otimes\langle e_i |\biggr) |\psi \rangle .
\end{equation*}
Then, the sum in the round brackets is an element of $\mathcal{H}\otimes\mathcal{H}^*$. While $\mathcal{H}\otimes\mathcal{H}^*$ is isomorphic to $\End(\mathcal{H})$, its elements are maps $\mathcal{H}^*\times\mathcal{H}\to\C$. Hence, one needs to either make this isomorphism explicit or, equivalently,
\begin{equation*}
|\psi \rangle
