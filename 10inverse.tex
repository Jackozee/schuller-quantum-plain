...
The information of a projection-valued measure \(P\) is completely
captured in a family of complex-valued measures \(\mu_{\psi,\phi}\),
parametrized by pairs of vectors \(\psi,\phi\in\Cal H\). We define
\begin{align}
  \mu_{\psi,\phi}\colon\sigma(\Cal O_R) &\to \C \\
  \Omega &\mapsto \mu_{\psi,\phi}(\Omega)
  := \langle \psi,P(\Omega)\phi\rangle
.\end{align}
By polarization, this is also captured by an associated family of
real-valued measures
\begin{align}
  \mu_{\psi}=\mu_{\psi,\psi}\colon\sigma(\Cal O_R) &\to \C \\
  \Omega &\mapsto \mu_{\psi,\psi}(\Omega) = \langle \psi,P(\Omega)\psi\rangle
.\end{align}

Now, how do we integrate with respect to a projection-valued measure?
For any bounded measurable function \(f\colon\R\to\C\), we will define
an integral
\begin{equation}
  F = \int_{\R} f\d P
,\end{equation}
which will be a bounded operator \(F\in\Cal L(\Cal H)\).
However, we will need more. For example, in the spectral theorem we
will need \(f\) to be the inclusion \(f\colon\R\to\C\).
