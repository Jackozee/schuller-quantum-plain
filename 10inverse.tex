...
We want to preserve property \eqref{eq:fmupsi}, i.e.
\begin{equation}
  \Big\langle \psi,\Big(\int_Mf\d P\Big) \psi\Big\rangle
  =
  \int_M f\d\mu_\psi
.\end{equation}
However, if \(f\) is unbounded, the right hand side may be infinite,
so the equation makes no sense, since the left hand side is always
finite.
Hence, given a measurable \(f\colon M\to\C\),
then we restrict the domain of \(\int_Mf\d P\) to be those \(\psi\)
such that \(f\) is \(\mu_\psi\)-integrable:
\begin{equation}
  \Cal D_{\int_Mf\d P}
  =
  \left\{
...
\end{equation}
is defined by taking a Cauchy sequence \(f_n\to f\) of bounded
integrable functions \(f_n\) that converge to \(f\) pointwise and that
form a Cauchy sequence in \(\Cal B(M)\), (this is impossible!)
and then defining
\begin{equation}
  \Big(\int_{M}f\d P\Big)\psi
  =
  \lim_{n\to\infty} \Big(\int_{M}f_n\d P\Big)\psi
\end{equation}
for every \(\psi\in\Cal D_{\int_M f\d P}\).
Note that \((\int_M f_n\d P)\psi\) is a Cauchy sequence, since
\begin{align}
  \Big\|
  \Big(\int_{M}f_m\d P\Big)\psi - \Big(\int_{M}f_n\d P\Big)\psi
  \Big\|
  &= \Big\| \Big(\int_{M}(f_m-f_n)\d P\Big)\psi \Big\| \\
  &\leq \|f_m-f_n\| \cdot \|\psi\|,
\end{align}
so the limit on the right hand side exists in \(\Cal H\).
One must show that this definition is independent of the sequence
\(f_n\), but once this is done we have the desired property, by
dominated convergence:
\begin{align}
  \left\langle \psi, \Big(\int_{M}f\d P\Big)\psi \right\rangle
  &=
  \left\langle
    \psi, \lim_{n\to\infty} \Big(\int_{M}f_n\d P\Big)\psi
  \right\rangle \\
  &= \lim_{n\to\infty}
  \left\langle
    \psi, \Big(\int_{M}f_n\d P\Big)\psi
  \right\rangle \\
  &= \lim_{n\to\infty} \int_{M}f_n\d\mu_\psi \\
  &= \int_{M}f\d\mu_\psi
\end{align}
Now, we would like to show that, for \(f\) measurable, the domain of
the operator \(\int_Mf\d P\) is a dense subset of \(\Cal H\).
Once we do this, the main problem will be to check that 
