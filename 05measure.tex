...
This and the next section will be a short recap of basic notions from
measure theory and Lebesgue integration. These are inescapable
subjects if one wants to understand quantum mechanics since

\begin{enumerate}[label=(\roman*)]
\item the spectral theorem requires the notion of (projection-valued)
  measures
\item the most commonly emerging separable Hilbert space in quantum
  mechanics is the space $L^2(\R^d)$, whose definition needs the
  notion of Lebesgue integral.
\end{enumerate}

\section{General measure spaces and basic results}
...
\begin{proposition}
Let $M$ be a set and let $\Sigma$ be a $\sigma$-algebra on $M$. Let $\{ A_n \}_{n\in \N}$ be a sequence in $\Sigma$. Then, for all $k\in \N$, we have
\begin{enumerate}[label=(\roman*)]
...
\begin{equation*}
B_n = \begin{cases} A_n \quad &\text{if }\ 0 \leq n \leq k\\
 \varnothing \quad &\text{if }\ n > k. \end{cases} 
\end{equation*}
...
\begin{definition}
A \emph{measurable space} is a pair $(M,\Sigma)$ where $M$ is a set
and $\Sigma$ is a $\sigma$-algebra on $M$. The elements of $\Sigma$
are called \emph{measurable subsets} of $M$.
\end{definition}

Our goal is to assign volumes (i.e.\ measures) to subsets of a given
set. Of course, we would also like this assignment to satisfy some
sensible conditions. However, it turns out that one cannot sensibly
assign volumes to any arbitrary collection of subsets of a given set%
\footnote{\url{https://en.wikipedia.org/wiki/Non-measurable_set}}.
It is necessary that the collection of subsets be a $\sigma$-algebra.
In addition, just like in topology openness and closeness are not
properties of subsets but properties of subsets with respect to a
choice of topology, so does measurability of subsets only make sense
with respect to a choice of $\sigma$-algebra. In particular, a given
subset could be measurable with respect to some $\sigma$-algebra and
not measurable with respect to some other $\sigma$-algebra.

\begin{example}
The pair $(M,\mathscr{P}(M))$ is a measurable space for any set $M$.
Of course, just like the discrete topology is not a very useful
topology, the power set $\mathscr{P}(M)$ is not a very useful
$\sigma$-algebra on $M$, unless $M$ is countable.
\end{example}

\begin{definition}
The \emph{extended real line}\index{extended real line} is
$\overline{\R} := \R \cup \{-\infty,+\infty\}$, where the symbols
$-\infty$ and $+\infty$ (the latter often denoted simply by $\infty$)
satisfy
\begin{equation*}
...
A sequence $\{A_n\}_{n\in \N}$ that satisfies the condition that $A_i
\cap A_j = \varnothing$ for all $i \neq j$ is called a \emph{pairwise
disjoint} sequence.
\begin{remark}
...
\begin{example}
  Let $M=\N$ and $\Sigma=\mathscr{P}(\N)$. Define $\mu : \Sigma \to
  [0,\infty]$ by:
  \begin{equation*}
  \mu(A) = |A|:=
  \begin{cases}
  ...
  \end{cases}
  \end{equation*}
  Then, by definition, $\mu(\varnothing) =0$. Moreover, if $\{A_n\}_{n\in\N}$
  is a pairwise disjoint sequence in $\Sigma$ such that all but a
  finite number of $A_n$'s are empty and each $A_n$ has a finite
  number of elements, we have:
  \begin{equation*}
  \mu \biggl( \bigcup_{\,n=0}^{\infty}A_n\biggr) = \sum_{n=0}^{\infty}\mu(A_n)
  \end{equation*}
  by counting elements. Otherwise, that is, if an infinite number of
  $A_n$'s are non-empty or if at least one $A_n$ is infinite, then
  \begin{equation*}
  \mu \biggl( \bigcup_{\,n=0}^{\infty}A_n\biggr)
  = \infty = \sum_{n=0}^{\infty}\mu(A_n)
  \end{equation*}
...
\begin{proof}
\begin{enumerate}[label=(\roman*)]
\item Define $B_0:=A_0$ and $B_n := A_n\setminus A_{n-1}$. Then, $\{ B_n \}_{n\in\N}$ is a pairwise disjoint sequence in $\Sigma$ such that \begin{equation*}
...
\begin{definition}
Let $(M,\Sigma,\mu)$ be a measure space. The measure $\mu$ is said to
be \emph{finite}\index{finite measure} if there exists a sequence
$\{A_n\}_{n\in\N}$ in $\Sigma$ such that $\bigcup_{n=0}^{\infty}A_n=M$
and
\begin{equation*}
...
\section[\texorpdfstring{Borel $\sigma$-algebras}{Borel sigma-algebras}]{Borel $\sigma$-algebras}
...
\begin{proposition}
Let $M$ be a set and let $\{ \Sigma_i : i \in I\}$ be a collection of
$\sigma$-algebras on $M$. Define the set
\begin{equation*}
\Sigma :=\bigcap_{i\in I}\Sigma_i = \{ A \in \mathscr{P}(M) \mid A \in \Sigma_i, \forall \, i \in I \}.
\end{equation*}
Then, $\Sigma$ is a $\sigma$-algebra on $M$.
\end{proposition}
\begin{proof}
...
and, since each $\Sigma_i$ is a $\sigma$-algebra, we also have
$M\setminus A \in \Sigma_i$ for all $i\in I$. Hence, $M\setminus A \in
\Sigma$.
...
\end{equation*}
Hence, $\sigma(\mathcal{O}_{\R})$ contains, in addition to all open
intervals, also all half-open intervals. It is not difficult to show
that it contains all closed intervals as well. In particular, since
singletons are closed, $\sigma(\mathcal{O}_{\R})$ also contains all
countable subsets of $\R$. In fact, it is non-trivial%
\footnote{\url{https://en.wikipedia.org/wiki/Borel_set\#Non-Borel_sets}}
to produce a subset of $\R$ which is not contained in
$\sigma(\mathcal{O}_{\R})$.
\end{example}

\section[\texorpdfstring{Lebesgue measure on $\R^d$}{Lebesgue measure on R\textasciicircum d}]{Lebesgue measure on $\R^d$}
...
This can easily be generalised to show that $\lambda^d$ is finite for
all $d\geq 1$.

\section{Measurable maps}

As we have remarked earlier, once we introduce a new structure, we
should immediately think about the associated structure-preserving
maps. In the case of measurable spaces, a measurable map is one that
preserves the ``measurability'' structure.
...
\section{Push-forward of a measure}
