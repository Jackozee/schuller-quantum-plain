...
If \(A\in\Cal L(\Cal H,\Cal H)\) is a \emph{bounded} operator, then
for every
...
there is a \emph{bounded} \(A^*\colon\Cal H\to\Cal H\) such that
\begin{equation}
  \langle \psi\mid A\alpha\rangle
  =
  \langle A^*\psi\mid \alpha\rangle.
\end{equation}
One can prove that \(A^*\) is the unique operator with this
property, and we call it the \emph{adjoint} of \(A\).
If \(A=A^*\), we say that \(A\) is self-adjoint.

For various reasons, the observables in quantum mechanics are modeled
by \emph{self-adjoint operators}. However, most of them are \emph{not
bounded}. It turns out that not-bounded operators defined on all of
\(\Cal H\) cannot be self-adjoint so, in order to have self-adjoint
operators, we will need to work with operators which are only defined
on a \emph{dense} subspace \(\Cal D_A\subseteq\Cal H\).

\begin{definition}
  A \emph{densely defined operator on \(\Cal H\)} is a
  linear map $A\cl\mathcal{D}_A\to\mathcal{H}$,
  where $\mathcal{D}_A\subseteq\Cal H$ is a dense subspace, i.e.
  \begin{equation*}
  ...
  \end{equation*}
  Equivalently, if for every $\psi\in\mathcal{H}$ there exists a
...
\end{align*}
is not necessarily bounded, so we cannot use the Riesz theorem to
put it in the form \(f_\eta\) for some \(\eta\in\Cal D_A\).
However, if there is such an \(\eta\), then the operator \(f_\psi\circ
A\) is necessarily bounded.
...
\begin{definition}
  ...
  \begin{enumerate}[label=(\roman*)]
    \item
      ...
      \end{align}
    \item
      Given a \(\psi\in\Cal D_{A}\), the operator \(f_\psi\circ
      A\colon \Cal D_A\to\C\) is bounded and densely defined. By the
      BLT theorem (\cref{thm:BLT}), \(f_\psi\circ A\) admits a unique
      ...
      \(\eta\in\Cal H\). Writing $A^*\psi:=\eta$, we obtain a linear
      operator \(A^*\colon\Cal D_{A^*}\to\Cal H\).
      In other words, \(A^*\psi\) is the unique element of \(\Cal H\)
      satisfying
      \begin{equation}
...
\begin{remark}
  By the brief discussion in part (ii) of the definition, the domain
  of \(A^*\) admits the following descriptions
  \begin{align}
    \mathcal{D}_{A^*}
    &=
    \{
...
\begin{remark}
  ...
  If \(\Cal D_{A^*}\) happens to be dense, we say that \(A\) is
  \emph{closable} (see \cref{sec:closable-closure-closed}).
\end{remark}
...
\begin{definition}
  Let \(A\colon\Cal D_A\to\Cal H\) be a densely defined operator. Then
  \begin{enumerate}[label=(\roman*)]
    \item $A$ is called \emph{closable} if
      its adjoint $A^*$ is also densely defined.
    \item If \(A\) is closable, its \emph{closure} is the operator
      $\overline{A}:=A^{**}$.
    \item A closable operator is called \emph{closed}\index{closed
      operator} if $A=\overline{A}$.
  \end{enumerate}
\end{definition}
...
\begin{lemma}
  ...
  Let $A$ be a closable operator. Then
  \begin{enumerate}
    \item $A\subseteq A^{**}$,
    \item \(A^*\) is closable and, in fact, closed,
    \item \(A^{**}\) is closed and, in fact, it is the smallest closed extension of \(A\).
  \end{enumerate}
\end{lemma}
...
\begin{equation*}
  ...
  \langle A\psi | \alpha \rangle,
\end{equation*}
so \(\psi\in\Cal D_{A^{**}}\) and $A^{**}\psi= A\psi$.
Thus, $A\subseteq A^{**}$, as we wanted.

To see that \(A^{*}\) is closable, note that,
by part (i), we have \(A\subseteq A^{**}\),
so the domain of \(A^{**}\) contains the domain of \(A\), which
implies that \(A^{**}\) is densely defined.
Since \(A^{*}\) is closable, we have \(A^{*}\subseteq A^{***}\) by
part (i). To see that \(A^{*}\) is closed, it suffices to see that the
containment \(\Cal D_{A^*}\subseteq\Cal D_{A^{***}}\) is an equality.
Take \(\psi\) in the domain of \(A^{***}\), which
means that the operator
\begin{equation}
  f_\psi\circ A^{**} \colon \Cal D_{A^{**}} \to \C
\end{equation}
is bounded. Restricting to \(\Cal D_A\subseteq \Cal D_{A^{**}}\), we
see that 
\begin{equation}
  f_\psi\circ A \colon \Cal D_{A} \to \C
\end{equation}
is bounded, where we used that \(A\subseteq A^{**}\).
Hence, \(\psi\in\Cal D_{A^*}\), as we wanted.

To see part (iii), first observe that \(A^{**}\) is closed, by part
(ii). Now suppose that \(B\) is another closed extension of \(A\),
i.e. \(B\) is closed and \(A\subseteq B\).
By applying \cref{prp:adjointreverse} twice, we see that
