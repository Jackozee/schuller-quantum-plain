...
\section{Closed operators, closability, closure}
\label{sec:closable-closure-closed}

\begin{definition}
  An operator \(A\colon\Cal D_A\to\Cal H\) (not necessarily densely
  defined) is \emph{closed} if its graph
  \begin{equation}
    \Gamma_A
  ...
  In terms of sequences, this means that \(A\) is closed if, for each
  sequence \(\{\psi_n\}\) in \(\Cal D_A\) with \(\psi_n\to\psi\in\Cal
  H\) and \(A\psi_n \to \alpha\in\Cal H\), it follows that
  \(\psi\in\Cal D_A\) and \(A\psi=\alpha\).

  The operator \(A\) is called \emph{closable} if it has a closed
  extension.
\end{definition}
...
We would like to see that every closed operator \(A\) has a minimal
closed extension, called its closure.
Suppose \(A\colon\Cal D_A\to\Cal H\) is closable and consider the space
\begin{equation}
  \Cal D_{\overline A}
  = \{\psi\in\Cal H \mid
    \exists \{\psi_n\}\in\Cal D_A, \
    \exists \alpha\in\Cal H, \
    \psi_n\to \psi, A\psi_n\to \alpha\}
\end{equation}
and note that \(\Cal D_A\subseteq \Cal D_{\overline A}\subseteq
\overline{\Cal D_A}\).
We want to define a new operator \(\overline A\colon \Cal D_{\overline
A}\to\Cal H\) by \(\overline A\psi = \alpha\).
Note that, once we pick a sequence \(\{\psi_n\}\), the element
\(\alpha\) is completely determined by \(A\psi_n\to\alpha\),
so we only must show that, given \(\psi\in\Cal D_{\overline A}\), all
the sequences \(\{\psi_n\}\) are associated to the same \(\alpha\).
This is why we need \(A\) to be closable.

Take sequences \(\{\psi_n\},\{\psi'_n\}\) in \(\Cal D_A\) with
\(\psi_n\to\psi\), \(\psi'_n\to\psi\), and suppose that there are
\(\alpha,\alpha'\in\Cal H\) with \(A\psi_n\to\alpha\),
\(A\psi'_n\to\alpha'\).
Since \(A\) is closable, we can take a closed extension \(B\) of
\(A\). Then we have \(B\psi_n=A\psi_n\to\alpha\) and
\(B\psi'_n=A\psi'_n\to\alpha'\), so since \(B\) is closed, it follows
that \(\psi,\psi'\in\Cal D_B\) and \(\alpha=B\psi=\alpha'\).
This shows that the definition \(\overline A\psi=\alpha\) is
independent of the sequence \(\{\psi_n\}\) and that \(\overline
A\subseteq B\) for any closed extension \(B\) of \(A\).
Moreover, we have \(A\subseteq \overline A\), since for any
\(\psi\in\Cal D_A\), the constant sequence \(\psi_n=\psi\) satisfies
\(\psi_n\to\psi\) and \(A\psi_n\to A\psi\), so \(\overline
A\psi=A\psi\).
\begin{proposition}
  Suppose that \(A\) is closable.
  The operator \(\overline A\colon\Cal D_{\overline A}\to\Cal H\),
  called the \emph{closure} of \(A\),
  is the minimal closed extension of \(A\).
\end{proposition}
\begin{proof}
  We already saw that \(A\subseteq \overline A\subseteq B\) for any
  closed extension \(B\) of \(A\), so it suffices to see that
  \(\overline A\) is closed.

  Let \(\psi\in\Cal D_{\overline A}\) and take a sequence \(\{\psi_n\}\)
  in \(\Cal D_{\overline A}\) such that \(\psi_n\to\psi\),
  \(\overline A\psi_n\to\alpha\) for some \(\alpha\in\Cal H\).
  Since each \(\psi_n\) is an element of \(\Cal D_{\overline A}
  \), in particular for each \(n\) 
  there is a sequence \(\{u_{(n),m}\}_m\) in \(\Cal D_A\) with
  \(u_{(n),m}\to \psi_n\) and \(Au_{(n),m}\to \overline A\psi_n\) as
  \(m\to\infty\).
  Taking \(u_n=u_{(n),n}\), we obtain a sequence \(\{u_n\}\) in \(\Cal
  D_A\) such that
  \begin{equation}
    \lim_{n\to\infty} u_n = \lim_{n\to\infty}\psi_n = \psi,
    \qquad
    \lim_{n\to\infty} Au_n = \lim_{n\to\infty}\overline A\psi_n = \alpha,
  ,\end{equation}
  which means that \(\psi\in\Cal D_{\overline A}\) and that
  \(\overline A\psi = \alpha\). Hence, \(\overline A\) is closed, as
  we wanted.
\end{proof}

An important example of closed operators are the adjoints of densely
defined operators.

\begin{proposition}
  \label{prop:adj-closed}
...
For densely defined operators, the adjoint provides us with a succint
characterization of closability and another construction of the
closure.

\begin{proposition}
  \label{prop:closableext}
...
\begin{proof}
  The proof of (i) is tedious and we ommit it.
  We check (ii).
  ...
  and by part (i), \(B^{*}\) is also densely defined.
  Thus, we can apply \cref{prp:adjointreverse} twice to obtain
  \(A^{**}\subseteq B^{**}=B\), as we wanted.
\end{proof}

\begin{corollary}
  Let \(A\colon \Cal D_A\to\Cal H\) be a closable, densely defined
  operator. Then \((\overline A)^{*}=A^{*}\).
\end{corollary}
\begin{proof}
  Since \(A\subseteq \overline A\), the closure \(\overline A\) is
  densely defined, so we can take its adjoint.

  By the \cref{prop:closableext}, \(A^{*}\) is densely defined, and by
  \cref{prop:adj-closed} we also know it is closed.
  Hence, \(A^{*}=(A^{*})^{-}=A^{***}=(\overline A)^{*}\).
\end{proof}

\section{The adjoint and closure of a symmetric operator}
...
\emph{not} be symmetric. However, its closure is.
\begin{proposition}
  Let \(A\) be a symmetric operator. Then
  \begin{enumerate}[label=(\roman*)]
    \item \(A\) is closable,
    \item \(A\subseteq A^{**}\subseteq A^*\),
    \item \(\overline A=A^{**}\) is symmetric.
  \end{enumerate}
\end{proposition}
\begin{proof}
  Let's check (i).
  In \cref{prp:symext}, we saw that $A\subseteq A^*$.
  Since \(A\) is densely defined, \(A^{*}\) is also densely defined. 
  Hence, \(A\) is closable, by \cref{prop:closableext}.
...
\begin{definition}
A symmetric operator $A$ is called \emph{essentially
self-adjoint}\index{essentially self-adjoint operator} if
$\overline{A}$ is self-adjoint.
\end{definition}
...
\begin{proof}
  By definition, \(\overline A\) is a self adjoint extension of \(A\).
  Moreover, any self-adjoint extension \(B\) of \(A\) is closed, so
  \(A\subseteq \overline A\subseteq B\), but self-adjoint operators
  are maximal with respect to self-adjoint extensions, so \(\overline
  A=B\).
\end{proof}
...
\begin{theorem}
  ...
  If there is \(z\in\C\) such that
  \begin{equation}
    \label{eq:critselfadjoint}
    \ran(A+z)=\mathcal{H}=\ran(A+\overline{z}),
  \end{equation}
  then \(A\) is self-adjoint.
\end{theorem}

\begin{proof}
  Since $A$ is symmetric, by \Cref{prp:symext}, we have $A\subseteq
  A^*$. Hence, it suffices to show that \(\Cal D_{A^{*}}\subseteq \Cal
  D_A\).
  Take \(z\in\C\) satisfiyng \eqref{eq:critselfadjoint}.
  In particular, $\ran(A+\overline{z})=\mathcal{H}$,
  so for any \(\psi\in\Cal D_{A^{*}}\)
  there exists \(\alpha\in\Cal D_A\) such that
  \begin{equation*}
    (A+\overline{z})\alpha
    =
    A^*\psi+\overline{z}\psi.
  \end{equation*}
  ...
  \end{equation*}
  Hence, by positive-definiteness of the inner product, we have
  $\psi=\alpha\in\Cal D_A$, as we wanted.
\end{proof}
